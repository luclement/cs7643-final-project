...
def Conv2D(inputs, filters, kernel_size=3, strides=1, padding='VALID', Use_bias = None):
    if kernel_size == 3 and strides == 1:
        inputs = tf.pad(tensor=inputs, paddings=[[0, 0], [1, 1], [1, 1], [0, 0]], mode="REFLECT")
    if kernel_size == 7 and strides == 1:
        inputs = tf.pad(tensor=inputs, paddings=[[0, 0], [3, 3], [3, 3], [0, 0]], mode="REFLECT")
    if strides == 2:
        inputs = tf.pad(tensor=inputs, paddings=[[0, 0], [0, 1], [0, 1], [0, 0]], mode="REFLECT")
    # converted to TF2 function: https://www.tensorflow.org/api_docs/python/tf/keras/layers/Conv2D
...
def dwise_conv(input, k_h=3, k_w=3, channel_multiplier=1, strides=[1, 1, 1, 1],
    ...
    input = tf.pad(tensor=input, paddings=[[0, 0], [1, 1], [1, 1], [0, 0]], mode="REFLECT")
    with tf.compat.v1.variable_scope(name):
        in_channel = input.get_shape().as_list()[-1]
        w = tf.compat.v1.get_variable('w', [k_h, k_w, in_channel, channel_multiplier],regularizer=None,initializer=tf.compat.v1.keras.initializers.VarianceScaling(scale=2.0))
        conv = tf.nn.depthwise_conv2d(input=input, filter=w, strides=strides, padding=padding, dilations=None, name=name, data_format=None)
        if bias:
...
def Unsample(inputs, filters, kernel_size=3):
    '''
        ...
        '''
    new_H, new_W = 2 * tf.shape(input=inputs)[1], 2 * tf.shape(input=inputs)[2]
...
def Downsample(inputs, filters = 256, kernel_size=3):
    ...
    new_H, new_W =  tf.shape(input=inputs)[1] // 2, tf.shape(input=inputs)[2] // 2
    inputs = tf.image.resize(inputs, [new_H, new_W])
...
def Conv2DTransposeLReLU(inputs, filters, kernel_size=2, strides=2, padding='SAME', Use_bias = None):
    ...
    # converted to TF2 function: https://www.tensorflow.org/api_docs/python/tf/keras/layers/Conv2DTranspose
...
def Separable_conv2d(inputs, filters, kernel_size=3, strides=1, padding='VALID', Use_bias = tf.compat.v1.zeros_initializer()):
    if kernel_size==3 and strides==1:
        inputs = tf.pad(tensor=inputs, paddings=[[0, 0], [1, 1], [1, 1], [0, 0]], mode="REFLECT")
    if strides == 2:
        inputs = tf.pad(tensor=inputs, paddings=[[0, 0], [0, 1], [0, 1], [0, 0]], mode="REFLECT")
    # replaced with TF2 function: https://www.tensorflow.org/api_docs/python/tf/keras/layers/SeparableConv2D
